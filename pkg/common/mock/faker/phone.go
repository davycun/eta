...
// 移动电话号码
func MobilePhoneShort(langs ...string) string {
	// lang: zh_CN --> 中文
	// lang: en_US --> 英文
	lang := randLang(langs)
	var phoneNumber string
...
// 固定电话号码
func TelPhoneShort(langs ...string) string {
	// lang: zh_CN --> 中文
	// lang: en_US --> 英文
	lang := randLang(langs)
	var telPhone string
	if lang == "zh_CN" {
		cityCode := CityCode() //随机区号
		phone7Num := fmt.Sprintf("%07d", Number(1, 9999999))
		phone8Num := fmt.Sprintf("%08d", Number(1, 99999999))
		if len(cityCode) == 3 {
			telPhone = cityCode + phone8Num
		} else {
			telPhone = cityCode + phone7Num
		}
	}
	return telPhone
}
