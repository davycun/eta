...
func TestCacheDelKeyPattern(t *testing.T) {
	...
	b, err := cache.Get(key1, &p2)
	assert.NoError(t, err)
	assert.True(t, b)

	b, err = cache.DelKeyPattern(key + "*")
	assert.NoError(t, err)
	assert.True(t, b)

	b, err = cache.Get(key1, &p3)
	assert.NoError(t, err)
	assert.False(t, b)
	assert.Empty(t, p3.IdNo)
	assert.Empty(t, p3.Name)
